...
# **Цель:** Исследование динамики взаимодействия двух видов: хищников и жертв
#
# ## Теоретическое введение
#
# Модель Лотки-Вольтерры — это фундаментальная математическая модель в экологии,
...
# - **Период колебаний** (для малых отклонений): $T \approx \frac{2\pi}{\sqrt{\alpha\gamma}}$
# - **Сдвиг фаз**: пик численности хищников отстаёт от пика численности жертв
#
...
# - β = 0.02 — скорость поедания жертв хищниками
# - δ = 0.01 — коэффициент конверсии пищи в хищников
# - γ = 0.3 — смертность хищников
#
# **Начальные условия:**
# - x₀ = 40.0 — начальная популяция жертв
# - y₀ = 9.0 — начальная популяция хищников
...
# ### 3. Производные (скорости изменения)
...
non_zero_idx = 2:length(freq_prey)
...
plot!(plt6[5], freq_prey[non_zero_idx], spectrum_prey[non_zero_idx], label=L"x", color=:green, linewidth=1.5,
...
# Функция для поиска первого пика
...
println("="^60)
println("\nСгенерированные графики сохранены в: plots/$(script_name)/")
println("Результаты анализа выведены в консоль.")
